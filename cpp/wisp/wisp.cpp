...
void replace_substring(std::string& src, std::string substr,
                       std::string replacement) {
  size_t i = 0;
  for (i = src.find(substr, i); i != std::string::npos;
       i = src.find(substr, i)) {
...
bool is_symbol(char ch) {
  return (isalpha(ch) || ispunct(ch)) && ch != '(' && ch != ')' && ch != '"' &&
         ch != '\'';
}
...
class Value {
 public:
  ...
  // This function finds the remainder of two lisp values, and returns the lisp
  // value result.
  Value operator%(Value other) const {
    // If the other value's type is the unit type,
    // don't even bother continuing.
...
#else
      case INT:
        // If we do not support libm, we have to throw errors for floating point
        // values.
        if (other.type != INT)
...
Error::Error(Value v, Environment const& env, const char* msg)
    : env(env), msg(msg) {
  cause = new Value;
  *cause = v;
}
...
Value if_then_else(std::vector<Value> args, Environment& env) {
  if (args.size() != 3)
    throw Error(Value("if", if_then_else), env,
...
Value define(std::vector<Value> args, Environment& env) {
  if (args.size() != 2)
    throw Error(Value("define", define), env,
...
Value defun(std::vector<Value> args, Environment& env) {
  if (args.size() != 3)
    throw Error(Value("defun", defun), env,
                args.size() > 3 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value random(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 2)
    throw Error(Value("random", random), env,
                args.size() > 2 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value read_file(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 1)
    throw Error(Value("read-file", read_file), env,
...
Value include(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 1)
    throw Error(Value("include", include), env,
                args.size() > 1 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value eval(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 1)
    throw Error(Value("eval", eval), env,
                args.size() > 1 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
  else
...
Value subtract(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 2)
    throw Error(Value("-", subtract), env,
                args.size() > 2 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value remainder(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 2)
    throw Error(Value("%", remainder), env,
                args.size() > 2 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
  return args[0] % args[1];
}
...
Value greater_eq(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 2)
    throw Error(Value(">=", greater_eq), env,
                args.size() > 2 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
  return Value(int(args[0] >= args[1]));
}
...
Value get_type_name(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 1)
    throw Error(Value("type", get_type_name), env,
                args.size() > 1 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value cast_to_int(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 1)
    throw Error(Value(INT_TYPE, cast_to_int), env,
                args.size() > 1 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value index(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 2)
    throw Error(Value("index", index), env,
                args.size() > 2 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value insert(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 3)
    throw Error(Value("insert", insert), env,
                args.size() > 3 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value remove(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 2)
    throw Error(Value("remove", remove), env,
                args.size() > 2 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value head(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 1)
    throw Error(Value("head", head), env,
                args.size() > 1 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
  std::vector<Value> list = args[0].as_list();
  if (list.empty())
...
Value tail(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 1)
    throw Error(Value("tail", tail), env,
                args.size() > 1 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value parse(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 1)
    throw Error(Value("parse", parse), env,
                args.size() > 1 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value replace(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 3)
    throw Error(Value("replace", replace), env,
                args.size() > 3 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value display(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 1)
    throw Error(Value("display", display), env,
                args.size() > 1 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value debug(std::vector<Value> args, Environment& env) {
  ...
  if (args.size() != 1)
    throw Error(Value("debug", debug), env,
                args.size() > 1 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value Environment::get(std::string name) const {
  // Meta operations
  if (name == "eval")
    return Value("eval", builtin::eval);
  if (name == "type")
    return Value("type", builtin::get_type_name);
  if (name == "parse")
    return Value("parse", builtin::parse);

  // Special forms
  if (name == "do")
    return Value("do", builtin::do_block);
  if (name == "if")
    return Value("if", builtin::if_then_else);
  if (name == "for")
    return Value("for", builtin::for_loop);
  if (name == "while")
    return Value("while", builtin::while_loop);
  if (name == "scope")
    return Value("scope", builtin::scope);
  if (name == "quote")
    return Value("quote", builtin::quote);
  if (name == "defun")
    return Value("defun", builtin::defun);
  if (name == "define")
    return Value("define", builtin::define);
  if (name == "lambda")
    return Value("lambda", builtin::lambda);

  // Comparison operations
  if (name == "=")
    return Value("=", builtin::eq);
  if (name == "!=")
    return Value("!=", builtin::neq);
  if (name == ">")
    return Value(">", builtin::greater);
  if (name == "<")
    return Value("<", builtin::less);
  if (name == ">=")
    return Value(">=", builtin::greater_eq);
  if (name == "<=")
    return Value("<=", builtin::less_eq);

  // Arithmetic operations
  if (name == "+")
    return Value("+", builtin::sum);
  if (name == "-")
    return Value("-", builtin::subtract);
  if (name == "*")
    return Value("*", builtin::product);
  if (name == "/")
    return Value("/", builtin::divide);
  if (name == "%")
    return Value("%", builtin::remainder);

  // List operations
  if (name == "list")
    return Value("list", builtin::list);
  if (name == "insert")
    return Value("insert", builtin::insert);
  if (name == "index")
    return Value("index", builtin::index);
  if (name == "remove")
    return Value("remove", builtin::remove);

  if (name == "len")
    return Value("len", builtin::len);

  if (name == "push")
    return Value("push", builtin::push);
  if (name == "pop")
    return Value("pop", builtin::pop);
  if (name == "head")
    return Value("head", builtin::head);
  if (name == "tail")
    return Value("tail", builtin::tail);
  if (name == "first")
    return Value("first", builtin::head);
  if (name == "last")
    return Value("last", builtin::pop);
  if (name == "range")
    return Value("range", builtin::range);

  // Functional operations
  if (name == "map")
    return Value("map", builtin::map_list);
  if (name == "filter")
    return Value("filter", builtin::filter_list);
  if (name == "reduce")
    return Value("reduce", builtin::reduce_list);

// IO operations
#ifdef USE_STD
  if (name == "exit")
    return Value("exit", builtin::exit);
  if (name == "quit")
    return Value("quit", builtin::exit);
  if (name == "print")
    return Value("print", builtin::print);
  if (name == "input")
    return Value("input", builtin::input);
  if (name == "random")
    return Value("random", builtin::random);
  if (name == "include")
    return Value("include", builtin::include);
  if (name == "read-file")
    return Value("read-file", builtin::read_file);
  if (name == "write-file")
    return Value("write-file", builtin::write_file);
#endif

  // String operations
  if (name == "debug")
    return Value("debug", builtin::debug);
  if (name == "replace")
    return Value("replace", builtin::replace);
  if (name == "display")
    return Value("display", builtin::display);

  // Casting operations
  if (name == "int")
    return Value("int", builtin::cast_to_int);
  if (name == "float")
    return Value("float", builtin::cast_to_float);
